...
public class CommandNode extends Node {
    ...
    private static void printResults(int results, MatchTable[] tables) {
        ...
        for (int i = 0; i < resultSize; i++) {
            ...
            System.out.println(String.format("%2d) %s: %f", i,
                    tables[i].referenceImageID(), tables[i].getScore()));
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        QueryInitiator qi;
        if (Cuda.available()) {
            qi = cbir.getFactory().createQueryInitiator(
                    new URI(baseURI), stores, true, Cuda.getHandle(0));
        } else {
            qi = cbir.getFactory().createQueryInitiator(
                    new URI(baseURI), stores, true);
        }
