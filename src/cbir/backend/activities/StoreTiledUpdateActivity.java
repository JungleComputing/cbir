...
public class StoreTiledUpdateActivity extends StoreActivity {
    ...
    public void process(Event e) throws Exception {
        if (e.data instanceof Metadata) {
            ...
            if (md.getEndmembers() == null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("EndmemberExtraction for " + md.getImageID()
                            + " has failed ");
                }
            } else {
                getStore().put(md,
                        mai.getStoresForOriginalImage(md.getImageID()));
                receivedMetadata++;
                if (logger.isDebugEnabled()) {
            ...
            /*
             * if (mai.size() == receivedMetadata) { if
             * (logger.isDebugEnabled()) {
             * logger.debug("Acquired all metadata"); } finish(); } else {
             * suspend(); }
             */
            // TODO this activity never quits: we just stay alive to receive all
            // metadata,
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(String.format("Received a SingleArchiveIndex containing %d images", sai.size()));
                logger.debug(String.format("Store now knows %d images", mai.size()));
            }
            ...
            suspend();
        } else if (e.data instanceof MultiArchiveIndex) {
            // merge repositoryIndices

            MultiArchiveIndex remoteMai = (MultiArchiveIndex) e.data;
            // FIXME check this
            mai.merge(remoteMai);
            
            if (logger.isDebugEnabled()) {
                logger.debug(String.format("Received a MultiArchiveIndex containing %d images", remoteMai.size()));
                logger.debug(String.format("Store now knows %d images", mai.size()));
            }
    ...
    private void startFetchMetadata() {
        ...
        for (Entry<ImageIdentifier, Set<String>> element : mai.getUUIDIndex()
                .entrySet()) {
            getExecutor().submit(
