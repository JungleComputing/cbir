...
public class ControlActivity extends Activity {
	...
	public ControlActivity(Controller controller) {
		super(new UnitActivityContext(ContextStrings.GUI), true, true);
	...
	public void process(Event e) throws Exception {
		if (e instanceof FloatImageEvent) {
			...
			System.out.println("Received a QueryResultEvent, #elements: " + ((MatchTable[])e.data).length);
			logger.debug("Received a QueryResultEvent, #elements: " + ((MatchTable[])e.data).length);
			controller.deliverResult(((QueryResultEvent) e).getResults());
			suspend();
		} else if (e instanceof StoreIndexEvent) {
			System.out.println("Received a StoreIndexEvent, #elements: " + ((MultiArchiveIndex)e.data).size());
			...
			logger.debug("Received a TerminationEvent");
				...
				// FIXME we should not receive this over here, but at the command node?
				finish();
		} else {
