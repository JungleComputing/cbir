/************************************************************************************************
 *	Wrapper for blas and lapack functions
 ************************************************************************************************/
...
// extern int isamin_(int *n, float *x, int *incx);
...
void Scopy(int N, float *SRC, int INCSRC, float *DST, int INCDST){
	...
	#ifdef __MKL
		cblas_scopy(n, SRC, incsrc, DST, incdst); 
	#else
		scopy_(&n, SRC, &incsrc, DST, &incdst);
	#endif
...
void Sgemv(char TRANS, int M, int N, float ALPHA, float *A, int LDA, float *X, int INCX, float BETA, float *Y, int INCY ){
	...
	#ifdef __MKL
		cblas_sgemv(CblasColMajor, (trans=='t' ? CblasTrans : CblasNoTrans), m, n, alpha, A, lda, X, incx, beta, Y, incy);
	#else
...
void Dgemm(char TRANSA, char TRANSB, int M, int N, int K, double ALPHA, double *A, int LDA, double *B, int LDB, double BETA, double *C, int LDC ){
	...
	#ifdef __MKL
		cblas_dgemm(CblasColMajor, (transa=='t' ? CblasTrans : CblasNoTrans), (transb=='t' ? CblasTrans : CblasNoTrans), m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
	#else
...
void Ssyevr(char JOBZ, char RANGE, char UPLO, int N, float *A, int LDA, 
	...
	ssyevr_(&jobz, &range, &uplo, &n, A, &lda, 
			&vl, &vu, &il, &iu, &abstol,
			M, W, Z, &ldz, ISUPPZ, 
